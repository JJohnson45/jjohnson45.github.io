...
var sum = function(array) {
 ...
 //stop recursion
   ...
   //first number of array + the next first number since the first is cut off
...
var isEven = function(n) {
 //establish base will be true or false when -2 is complete
 if(n === 0){
   return true;
   //eventually subtraction will reach 0 or 1
 //must work with negative numbers
   ...
   //use recursion to subreact 2 from number argument until 0 or 1
   return isEven(n - 2);
 }
};
...
var sumBelow = function(n) {
 //add all sums below a number without adding the given number
 //subtract the given number and add it to total
 //end at base 0
 //total starts n-1
 var total = n - 1;
 var totalNeg = n + 1;
...
var range = function(x, y) {
 //base case
   if (x === y || y - 1 === x || x - 1 === y){
     return [];
   //return array with values between x and y
   } else if (x > y){
...
var palindrome = function(string) {
 //base case of being true if first and last indexes match and are taken out till the middle
 if (string.replace(/ /g,'').toLowerCase().length === 0 || string.replace(/ /g,'').toLowerCase().length === 1) {
   return true;
...
var createArray = function(str){
 ...
 const array = [];
 //set base case to return when string is empty
 if(str.length === 0) {
   return array;
 }
...
var numToText = function(str)  {
 ...
 var numRef = {0: 'zero', 1: 'one', 2: 'two', 3: 'three', 4: 'four', 5: 'five', 6: 'six', 7: 'seven', 8: 'eight', 9: 'nine', 10: 'ten'};
